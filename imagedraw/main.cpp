...
uint8_t byte_replace_bits(uint8_t source, uint8_t offset, uint8_t target, uint8_t target_len = 1) {
...
void set_graphics_mode(c64graphics_t graphics_mode) {
    ...
    *vbp = byte_replace_bits(*vbp, 0, !bmm);
    auto vmo = MAKE_VTPTR(uint8_t, 0xd018);
    *vmo = byte_replace_bits(*vmo, 3, bmm);
    *vmo = byte_replace_bits(*vmo, 5, bmm);
    *vmo = byte_replace_bits(*vmo, 6, bmm);
